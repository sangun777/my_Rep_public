...
def _make_deconv_layer(inputs, filters, strides=1, downsample=False, name=None):
    identity = inputs
    if downsample:
        identity = Conv2D(
...
def Simplebaseline(
    ...
    x = BatchNormalization(momentum=0.9)(x)
    x = ReLU()(x)
    x = _make_deconv_layer(x, 128, downsample=True)
    x = MaxPool2D(pool_size=2, strides=2)(x)
    x = _make_deconv_layer(x, 128, downsample=False)
    x = _make_deconv_layer(x, 256, downsample=True)

    ys = []
    for i in range(num_stack):
        x = HourglassModule(x, order=4, filters=256, num_residual=num_residual)
        for i in range(num_residual):
            x = _make_deconv_layer(x, 256, downsample=False)
